...
public public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
    //1.If root value is greater than both p and q values, then the root must be the lowest common ancestor.
    if(root.val > p.val && root.val > q.val){
        //1.1 Return the left subtree
        return lowestCommonAncestor(root.left, p, q);
    }
    //2.If root value is less than both p and q values, then the root must be the lowest common ancestor.
